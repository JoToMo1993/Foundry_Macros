...
function printCheck(number_of_minions, damage_formula) {
    let attackRoll = new Roll("1d20+" + number_of_minions).roll({async: false});
    game.dice3d?.showForRoll(attackRoll);

    let multipliedDamageFormular = multiply(damage_formula, number_of_minions);
    let damageRoll = new Roll(multipliedDamageFormular).roll({async: false});
    game.dice3d?.showForRoll(damageRoll);

    attackRoll.render({async: false}).then(attackRollContent => {
        damageRoll.render({async: false}).then(damageRollContent => {
            let message = `
                <div class="dnd5e2 chat-card item-card">
                    <section class="card-header description collapsible">
                        <header class="summary">
                            <div class="name-stacked border">
                                <span><strong>Minion's attack</strong> - ${number_of_minions}</span>
                            </div>
                        </header>
                    </section>
                    <section class="card-header description rsr-section-attack">
                        <div class="rsr-header">
                            <div class="rsr-title">
                                <dnd5e-icon src="systems/dnd5e/icons/svg/trait-weapon-proficiencies.svg"></dnd5e-icon>Attack
                            </div>
                        </div>
                        ${attackRollContent}
                    </section>
                    <section class="card-header description rsr-section-damage">
                        <div class="rsr-header">
                            <div class="rsr-title">
            ...
            let chatData = {
                user: game.user.id,
                speaker: {actor: canvas.tokens.controlled[0].actor.id},
                content: message,
            };
