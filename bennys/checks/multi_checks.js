...
await game.settings.register('multi_checks', 'type', {
    name: 'Type',
...
await game.settings.register('multi_checks', 'option', {
    name: 'Option',
...
await game.settings.register('multi_checks', 'dc', {
    name: 'DC',
...
const popupTypeTemplate = `<select name='type' id='type'>
  ${Object.keys(Types).map(typeKey => `<option value="${typeKey}" ${typeKey === game.settings.get('multi_checks', 'type') ? "selected" : ""}>${Types[typeKey].text}</option>`).join()}
</select>`;
...
function popupSpecificTemplate(typeKey) {
    return `<table>
      <thead>
        <tr><th>${Types[typeKey].text}</th></tr>
      </thead>
      <tbody>
        ...
        <tr class="${typeKey}-section">
          <td>
            <input type="checkbox" id="${typeKey + '-' + option.value}" name="checks" value="${option.value}"
                ${game.settings.get('multi_checks', 'option').includes(typeKey + '-' + option.value) ? "checked" : ""}
            />
...
function dialogTypeSubmit() {
    ...
    let dSpecific = new Dialog({
        ...
        content: popupSpecificTemplate(type),
        buttons: {
            ok: {
                icon: '<i class="fas fa-check"></i>',
                label: "Next!",
        ...
        <tr>
          ...
          <td>
            <input type="number" id="${check.type + '-' + check.optionValue}"
                value="${game.settings.get('multi_checks', 'dc')[check.type + '-' + check.optionValue] | ""}"
            />
          </td>
        </tr>`).join('')}
      </tbody>
...
async function dialogSubmit() {
    let savedDC = game.settings.get('multi_checks', 'dc');
    ...
    await game.settings.set('multi_checks', 'type', type);
    await game.settings.set('multi_checks', 'option', checks.map(check => check.type + '-' + check.optionValue));
    await game.settings.set('multi_checks', 'dc', savedDC);
