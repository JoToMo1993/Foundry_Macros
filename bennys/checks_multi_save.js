...
await game.settings.register('checks_multi_save', 'checks', {
    name: 'Checks',
    hint: 'Stores previously created check',
    scope: 'client',
    config: false,
    type: Object,
    default: {},
...
await game.settings.register('checks_multi_save', 'check-name', {
    name: 'Checkname',
    hint: 'Stores previously used check name',
    scope: 'client',
    config: false,
    type: String,
    default: "",
...
function typeCheck(typeKey) {
    const checkNameElement = document.getElementById("check-name")
    if (!checkNameElement) {
        return typeKey === 'skill';
    }
    const selected = checkNameElement.value;
    let savedChecks = game.settings.get('checks_multi_save', 'checks')[selected];
    if (!savedChecks) {
        return typeKey === 'skill';
    } else {
        return typeKey === savedChecks.type;
    }
}

function checkNamenList() {
    let checkNamen = Object.keys(game.settings.get('checks_multi_save', 'checks'));
    checkNamen.sort();
    return checkNamen.map(checkName => `<option value="${checkName}" />`).join('');
}

const popupTemplate = `
<table>
  <tr>
    <td><label for='check-name'>Check Name</label></td>
    <td>
      <input type="search" id="check-name" list="check-name-list"
        value="${game.settings.get('checks_multi_save', 'check-name')}">
  ...
  <tr>
    ...
    `<option value="${typeKey}">${Types[typeKey].text}</option>`).join('')}
      </select>
    </td>
  </tr>
...
function updateCheckName() {
    const selected = document.getElementById("check-name").value;
    const savedCheck = game.settings.get('checks_multi_save', 'checks')[selected];
    
    if (savedCheck) {
        document.getElementById("type").value = savedCheck.type;
        updateType();
        document.getElementById(savedCheck.type).value = savedCheck.valueOption;
        document.getElementById("dc").value = savedCheck.dc;
    }
}
document.getElementById("type").onchange = updateType
document.getElementById("check-name").onchange = updateCheckName
updateCheckName()
</script>
`;

async function dialogSubmit() {
    let checkName = document.getElementById("check-name").value;
    let type = document.getElementById("type").value;
    let valueOption = document.getElementById(type).value;
    let dc = document.getElementById("dc").value;

    // Save to local storage
    let savedChecks = game.settings.get('checks_multi_save', 'checks');
    savedChecks[checkName] = {
        type: type,
        valueOption: valueOption,
        dc: dc
    }
    game.settings.set('checks_multi_save', 'checks', savedChecks);
    game.settings.set('checks_multi_save', 'check-name', checkName);
...
let d = new Dialog({
    ...
    buttons: {
        ...
        clearAll: {
            ...
            label: "Clear",
            callback: () => {
                game.settings.set('checks_multi_save', 'checks', {});
                game.settings.set('checks_multi_save', 'check-name', "");
            }
        }
    }
});
