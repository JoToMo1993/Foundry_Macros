...
function printWeather(location, season, area, weather, wind, temp, additionalInformation) {
    Promise.all(Object.entries(weatherDiceRolls).map(async ([rollKey, roll]) => {
        ...
        let headerPart = createHeaderPart(location, season, area, weather, wind, temp);
...
async function weatherDiceMessage(weatherDiceRoll) {
    let content = await weatherDiceRoll.render();
    return weatherDiceRoll.toMessage({
        speaker: ChatMessage.getSpeaker(),
        content: content,
        rolls: Object.values(weatherDiceRoll).map(rollMessage => rollMessage.rolls).flat(1),
    }, {create: false})
}

function createHeaderPart(location, season, area, weather, wind, temp) {
    ...
    let name = `<header class="summary">${icon}<div class="name-stacked border"><span class="title">Weather</span><span class="subtitle">${LocationTranslate[location]} ${(location === "Moderate" ? ` - ${SeasonTranslate[season]}` : "")} - ${AreaTranslate[area]}</span></div></header>`;
    let weatherSection = `<section class="details card-content"><div class="wrapper"><div class="ddb"><p>${weather}</p><p>Windstärke: ${wind}</p><p>Temperatur: ${temp}°C</p></div></div></section>`;
